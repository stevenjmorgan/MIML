...
```{r load data}
# Load ANES data.
load("~/GitHub/MIML/ANES/cleanedANES.RData")
...
length(which(is.na(anes.knn1)))
length(which(is.na(anes.knn2)))
```
...
logit_knn <- glm(logit_form, data = anes.knn, family=binomial(link='logit'))
...
ols_knn <- lm(ols_form, data = anes.knn1)
